...
def load_crypto(pair, aggregation=5):
    ...
    tfvdf = tfv.vec(aggregation)
    fn_list = list(tfvdf.keys())
    fn_list.remove('target')
    fn_list.remove('close')
    data = tfvdf[fn_list].to_numpy(dtype=float)
    target = tfvdf['target'].to_numpy(dtype=float)
    feature_names = np.array(fn_list)
    target_names = np.array(t_f.TARGETS.keys())

    return Bunch(data=data, target=target,
                 target_names=target_names,
                 DESCR=tfv.cur_pair+', aggregation: '+str(aggregation),
                 feature_names=feature_names)
